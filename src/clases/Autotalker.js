...
export default class AutoTalker extends TextoAVoz {
  ...
  async init() {
    console.log('init autotalker');
    return new Promise(async (resolve, reject) => {
      try {
        await this.initConfigHandler();
        this.elementMonitor = new ElementMonitor(
          /(Stop generating)/i,
          /(Regenerate response|New response|There was an error generating a response|Generate new response)/i,
          'eventStartTalking',
          document,
          'FORM',
          200,
        );
        await this.initAutoTalker();
        resolve();
      } catch (error) {
        reject('error init autotalker: ', error);
      }
    });
  }
      ...
      if (drusPluginsDiv) {
        ...
        button.onclick = () => {
          ...
          this.configHandler
            ...
            .then(() => {
              button.innerHTML =
                newAutoTalk == 'always' ? this.talkIcon : this.silentIcon;
            })
            .catch((error) => {
        ...
        document.addEventListener(this.elementMonitor.eventName, () => {
          console.log(
            'order heard. checking config',
            this.configHandler.settings,
          );
          if (this.configHandler.settings.autoTalk == 'always') {
            console.log("Now I'll talk");
            const sectores = document.querySelectorAll(
              'main .group.bg-gray-50:nth-child(odd)',
            );
