...
export default class VozATexto {
  ...
  addListeningScreen() {
            ...
            <div class="d-flex justify-content-center flex-column gap-10">
                ...
                <div class="row d-flex flex-direction-row justify-content-between gap-20">
                    <div class="d-fd-column">
                        ...
                        <span class="text-small text-muted">${geti18nMessage(
                          'PressAToApprove',
                        )}</span>
                        
                    </div>
                    <div class="d-fd-column">
                        <button class="btn flex-center btn-dark restart">
                    ...
                    <div class="d-fd-column">
                        <button class="btn flex-center btn-danger cancelListening">
    ...
    this.speechToTextWindow(htmlContent).then(() => {
      console.log('theen');
      document
        .querySelector('.restart')
        .addEventListener('click', () => this.listen());
      document
        .querySelector('.cancelListening')
        .addEventListener('click', () => this.stopListening());
      document
        .querySelector('.aproveText')
        .addEventListener('click', () => this.aprovedText());
      this.addingScreenState = false;
    });
  }

  updateListeningElement(clase = 'activeListening') {
    const listeningDiv = document.querySelectorAll('.listeningState');
    if (listeningDiv.length > 0) {
      this.listeningDiv = true;
      listeningDiv.forEach((element) => {
        if (clase == 'activeListening') {
          element.classList.add('activeListening');
          element.classList.remove('inactiveListening');
        } else {
          element.classList.remove('activeListening');
          element.classList.add('inactiveListening');
        }
      });
    } else {
      this.listeningDiv = false;
  ...
  stopListening() {
    ...
    document
      .querySelectorAll('.speechToTextWindow')
  ...
  speechResult(event) {
    const transcriptElements = document.querySelectorAll('.transcript');
  ...
  async listen() {
    ...
    //await this.initConfigHandler();
    this.updateListeningElement('activeListening');
    ...
    try {
      ...
      this.recognition.onspeechend = (event) => {
        ...
        this.updateListeningElement('inactiveListening');
        this.recognition.stop();
      };
